...
function getTemperature(lat, lon) {
    const url = `https://archive-api.open-meteo.com/v1/era5?latitude=${lat}&longitude=${lon}&start_date=1960-01-01&end_date=2021-12-31&daily=temperature_2m_max&timezone=Europe%2FBerlin`;
    fetch(url)
        .then(response => response.json())
        .then(data => {
            const historical = {};
